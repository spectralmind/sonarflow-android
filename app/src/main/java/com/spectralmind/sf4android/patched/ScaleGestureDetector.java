...
package com.spectralmind.sf4android.patched;
...
public class ScaleGestureDetector {
    ...
    public static class SimpleOnScaleGestureListener implements OnScaleGestureListener {
        ...
        public void onScaleEnd(ScaleGestureDetector detecto) {
            // Intentionally empty
        }
...
//    private float startFocusY;
    ...
    private float minDelta = 2.5f; // best value from experience
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (!mGestureInProgress) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
            	...
            	if(event.getActionIndex() > 1) // ignore third finger 
            		break;
...
//                	mFocusY = event.getY(id);
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if(mCurrPressure / mPrevPressure > PRESSURE_THRESHOLD)
                    {
                    	if(ifScaleScroll())
                    	{
                    		mListener.onScalesScroll(-1*(dx0+dx1)/2,-1*(dy0+dy1)/2);
                    	}
                    	...
                    	mPrevEvent.recycle(); //refresh for next call
                    	mPrevEvent = MotionEvent.obtain(event); //copy actuall event
    ...
    private boolean iro(float f,double rangeMin,double rangeMax) //iro = in range of
    {
    ...
    private boolean ifScaleScroll()
    {
    	if(iro(getCurrentSpan(),getPreviousSpan()-minDelta,getPreviousSpan()+minDelta)
    			|| iro(getPreviousSpan(),getCurrentSpan()-minDelta,getCurrentSpan()+minDelta))
    		{
        		return true;
    		}    	
    	return false;
	...
	public void adjustminDeltaTofitDevice(float i) {
		this.minDelta = this.minDelta*i;
	}
}